...
We try our best to allow any project to be plugged in. However, we have many
assumptions about the code we parse coming from our background of parsing the
Linux kernel.

Elixir's assumptions:

* C code.
* Project sources have to be available in one git repository (or more than one).
* A release is a Git tag.

First make an installation of Elixir by following the above instructions. Docker
is recommended but it is doable without.

Once Elixir works for at least one project (`musl` is a good testing ground), it
is time to add your project. The list of existing projects is `ls projects/`.

Required steps are:

* Add one or more remotes in `utils/index`.
* Create an empty `projects/<PROJECT_NAME>.sh`.

What follows is project specific details that you might need to fill in.

First off, tags parsing. If tags are not formatted correctly, or some should be
hidden, that is done with a custom `list_tags()` function inside
`projects/<PROJECT_NAME>.sh`. `git grep 'list_tags(' projects/` to find examples.

The `list_tags_h()` function does the same thing, but generates a tree of tags
to be used in the Elixir sidebar. It is three fields per line, with
`<top_menu>\t<sub_menu>\t<version>` format. Try your best to make both
`list_tags()` and `list_tags_h()` agree; most projects do it wrong.

On the same topic, `get_latest_tags()` is used to find the latest version. By
default it ignores "-rc" versions. You might want to tweak it to your project
to avoid beta builds.

In the same `projects/<PROJECT_NAME>.sh` file, setting `dts_comp_support=1`
enables `compatible` parsing in devicetree files.

Also, you might need to configure _filters_. They get applied to the HTML before
rendering, adding links & such. Defaults are sensible for C projects but you
might want to add support for Makefile, Kconfig, etc. That is done in
`elixir/filters/projects.py`.
