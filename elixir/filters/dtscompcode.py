...
class DtsCompCodeFilter(Filter):
    ...
    def check_if_applies(self, ctx) -> bool:
        return (
            ...
            and extension_matches(
                ctx.filepath, {"c", "cc", "cpp", "c++", "cxx", "h", "s"}
            )
        )
    ...
    def transform_raw_code(self, ctx, code: str) -> str:
        ...
        compatible_search = re.search(
            "\.(\033\[31m)?compatible(\033\[0m)?\s*=", code, flags=re.MULTILINE
        )
        ...
        return re.sub(
            '(\s*{*\s*\.(\033\[31m)?compatible(\033\[0m)?\s*=\s*)"(.+?)"',
            keep_dtscompC,
            code,
            flags=re.MULTILINE,
        )
    ...
    def untransform_formatted_code(self, ctx: FilterContext, html: str) -> str:
        ...
        return re.sub(
            "__KEEPDTSCOMPC__([A-J]+)", replace_dtscompC, html, flags=re.MULTILINE
        )
